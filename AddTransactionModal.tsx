...
interface AddTransactionModalProps {
  ...
  initialData?: Partial<Omit<Transaction, 'id' | 'date' | 'icon'>>;
}
...
const AddTransactionModal: React.FC<AddTransactionModalProps> = ({ onClose, onSaveTransaction, transactionToEdit, initialType = 'expense', initialData }) => {
  ...
  const [type, setType] = useState<'income' | 'expense'>(() => {
    ...
    return initialData?.type || initialType;
  });
  ...
  return (
    ...
    >
      <div 
        className="bg-[rgb(var(--color-card-rgb))] rounded-2xl shadow-lg p-6 md:p-8 w-full max-w-md transition-colors"
