...
const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({
  ...
  const stats = useMemo(() => {
    ...
    const topCategory =
      categories.find((c) => c.id === topCatId)?.name || topCatId || 'None';
  ...
  // ── Net worth trend (cumulative, monthly) ───────────────────────────────────
  const netWorthData = useMemo(() => {
    ...
    sorted.forEach((t) => {
      ...
      byMonth[key] = (byMonth[key] || 0) + (t.type === 'income' ? t.amount : -t.amount);
    });
    let cumulative = 0;
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <KpiCard
              ...
              value={formatCurrency(stats.thisIncome, user.currency)}
              change={stats.incomeChange}
              positive
            />
            <KpiCard
              label="This Month Spending"
              value={formatCurrency(stats.thisExpense, user.currency)}
                ...
                <CardContent>
                  <div style={{ height: 280 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={momData} barCategoryGap="30%">
                        <CartesianGrid strokeDasharray="3 3" stroke={gridColor} />
                        <XAxis dataKey="label" tick={{ fontSize: 12, fill: chartTextColor }} axisLine={false} tickLine={false} />
                        <YAxis tick={{ fontSize: 12, fill: chartTextColor }} axisLine={false} tickLine={false}
                          tickFormatter={(v) => `${user.currency}${v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v}`}
                        />
                        <Tooltip
                          contentStyle={{
                        ...
                        <Bar dataKey="expense" name="Spending" fill={expenseColor} radius={[4, 4, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </CardContent>
      ...
      {activeTab === 'net worth' && (
        <div className="flex flex-col gap-4">
          {netWorthData.length < 2 ? (
            <EmptyState label="Not enough data for net worth trend" />
          ) : (
            <Card>
              <CardHeader>
              ...
              <CardContent>
                ...
                <div className="grid grid-cols-3 gap-3 mt-4">
                  {[
                    ...
                    {
                      label: 'Total Growth',
                      value: (netWorthData[netWorthData.length - 1]?.netWorth ?? 0) - (netWorthData[0]?.netWorth ?? 0),
                    },
                  ].map(({ label, value }) => (
                    <div key={label} className="rounded-xl p-3 flex flex-col gap-1" style={{ background: 'rgba(128,128,128,0.07)' }}>
                      ...
                      <span
                        ...
                        style={{ color: value >= 0 ? incomeColor : expenseColor }}
                      >
