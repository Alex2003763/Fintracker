...
const FloatingActionButton: React.FC<{ onClick: () => void }> = ({ onClick }) => (
  <button
    onClick={onClick}
    className="fixed z-40 bottom-28 right-6 md:bottom-8 md:right-8 h-16 w-16 bg-gradient-to-br from-[rgb(var(--color-primary-rgb))] to-[rgb(var(--color-primary-hover-rgb))] rounded-full flex items-center justify-center shadow-xl border-4 border-[rgb(var(--color-card-rgb))] hover:border-[rgba(var(--color-primary-rgb),0.3)] transition-all duration-300 ease-out transform hover:scale-110 active:scale-95 focus:outline-none focus:ring-4 focus:ring-[rgba(var(--color-primary-rgb),0.3)] group overflow-hidden aspect-square"
    aria-label="Add Transaction"
    aria-haspopup="dialog"
    style={{ boxShadow: '0 8px 32px rgba(0,0,0,0.18)', bottom: 'calc(7rem + env(safe-area-inset-bottom))' }}
  >
