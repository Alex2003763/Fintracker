...
export const Input: React.FC<InputProps> = memo(({
  ...
  const baseClasses = `
    block w-full px-3 py-3 sm:py-2.5
...
export const Button: React.FC<ButtonProps> = memo(({
  ...
  const sizeClasses = useMemo(() => ({
    ...
    lg: 'px-6 py-4 sm:py-3 text-base min-h-[48px]',
  }), []);
    ...
    <div className={`flex rounded-lg border p-1 bg-[rgb(var(--color-bg-rgb))] border-[rgb(var(--color-border-rgb))] ${className}`}>
      {options.map((option) => (
        <button
          ...
          className={`flex-1 py-3 sm:py-2 px-3 rounded-md text-sm font-semibold transition-all duration-200 min-h-[44px] ${
            value === option.value
...
export const LoadingState: React.FC<LoadingStateProps> = ({ message = 'Loading...' }) => {
   return (
     <div className="flex items-center justify-center py-8 sm:py-12">
       <div className="flex flex-col items-center space-y-4 text-[rgb(var(--color-text-muted-rgb))] text-center">
         <div className="relative">
           <svg className="animate-spin h-8 w-8 sm:h-10 sm:w-10" fill="none" viewBox="0 0 24 24">
             <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
             <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" />
           </svg>
           <div className="absolute inset-0 rounded-full bg-[rgb(var(--color-primary-rgb))]/10 animate-ping"></div>
         </div>
...
export const SuccessState: React.FC<SuccessStateProps> = ({
   ...
   return (
     <div className="flex flex-col items-center justify-center py-8 sm:py-12 text-center px-4">
       <div className="w-16 h-16 sm:w-20 sm:h-20 bg-green-100 rounded-full flex items-center justify-center mb-6 animate-in zoom-in duration-500">
         <svg className="w-8 h-8 sm:w-10 sm:h-10 text-green-600 animate-in fade-in slide-in-from-bottom-2 duration-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
           <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
         </svg>
       </div>
       <h3 className="text-lg sm:text-xl font-semibold text-[rgb(var(--color-text-rgb))] mb-3 sm:mb-4 leading-relaxed">
         {message}
       </h3>
       {onContinue && (
         <Button onClick={onContinue} className="mt-2 sm:mt-4 min-h-[48px]">
           Continue
         </Button>
       )}
     </div>
      ...
      <div className="flex space-x-0">
        {tabs.map((tab, index) => (
          <button
            ...
            className={`group relative flex items-center px-4 py-3 sm:py-2.5 text-sm font-medium transition-all duration-200 ease-out min-h-[44px] ${
              activeTab === tab.id
